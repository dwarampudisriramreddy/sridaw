...
try:
    ...
    try:
        ...
        debug_log(f"Jnius available but Android classes failed: {e}", "WARN")
except Exception as e:
    debug_log(f"Desktop environment (jnius not found): {e}")
...
def register_fonts():
    try:
        # Avoid complex font registration on startup
        debug_log("Using system default fonts for maximum stability")
        return True
    except Exception as e:
        debug_log(f"Font registration warning: {e}", "WARN")
        return False
...
<MainLayout>:
    ...
    BoxLayout:
        ...
        Button:
            ...
            on_press: app.show_save_chooser()

        Button:
            text: 'Load'
            size_hint_x: 0.1
            background_color: 0.4, 0.4, 0.7, 1
            on_press: app.show_load_chooser()
...
result = s'''
    ...
    def show_save_chooser(self, *args):
        try:
            ...
            fc = FileChooserIconView(path=path, dirselect=True)
            ...
            save_btn = Button(text='Save Code Here', background_color=(0.2, 0.8, 0.2, 1))
            
            buttons.add_widget(cancel_btn)
            buttons.add_widget(save_btn)
            content.add_widget(buttons)
            
            popup = Popup(title="Select Folder to Save Code", content=content, size_hint=(0.9, 0.9))
            
            cancel_btn.bind(on_release=popup.dismiss)
            save_btn.bind(on_release=lambda x: self.save_code(fc.path, popup))
            
            popup.open()
        except Exception as e:
            self.status_text = f"Chooser error: {e}"
            debug_log(f"Save chooser error: {e}", "ERROR")

    def save_code(self, folder_path, popup):
        try:
            self.last_path = folder_path
            filename = f"sridaw_code_{int(time.time())}.py"
            file_path = os.path.join(folder_path, filename)
            
            try:
                code = self.layout.ids.editor.text
            except:
                self.status_text = "Error: Could not access editor text"
                return
            ...
            self.status_text = f"Code saved to: {filename}"
            debug_log(f"Code saved to: {file_path}")
            popup.dismiss()
        except Exception as e:
            self.status_text = f"Save failed: {str(e)}"
            debug_log(f"Save error: {e}", "ERROR")

    def show_load_chooser(self, *args):
        try:
            if self.last_path and os.path.exists(self.last_path):
                path = self.last_path
            elif ANDROID:
            ...
            content = BoxLayout(orientation='vertical', spacing=dp(5), padding=dp(5))
            fc = FileChooserIconView(path=path, filters=['*.py'])
            content.add_widget(fc)
            
            buttons = BoxLayout(size_hint_y=None, height=dp(50), spacing=dp(10))
            cancel_btn = Button(text='Cancel', background_color=(0.8, 0.2, 0.2, 1))
            load_btn = Button(text='Load Selected File', background_color=(0.2, 0.5, 0.9, 1))
            
            buttons.add_widget(cancel_btn)
            buttons.add_widget(load_btn)
            content.add_widget(buttons)
            
            popup = Popup(title="Select .py File to Load", content=content, size_hint=(0.9, 0.9))
            
            cancel_btn.bind(on_release=popup.dismiss)
            load_btn.bind(on_release=lambda x: self.load_code(fc.selection, fc.path, popup))
            
            popup.open()
        except Exception as e:
            self.status_text = f"Chooser error: {e}"
            debug_log(f"Load chooser error: {e}", "ERROR")

    def load_code(self, selection, folder_path, popup):
        try:
            if not selection:
            ...
            self.layout.ids.editor.text = code
            self.status_text = f"Loaded: {os.path.basename(file_path)}"
            debug_log(f"Code loaded from: {file_path}")
            popup.dismiss()
            
            # Optionally run the code immediately
            Clock.schedule_once(lambda dt: self.run_code(), 0.5)
        except Exception as e:
            self.status_text = f"Load failed: {str(e)}"
            debug_log(f"Load error: {e}", "ERROR")
    ...
    def play_audio(self, *args):
        """
        Plays the current music stream. 
        Direct memory-to-audio playback for MIDI isn't natively supported 
        by Android's MediaPlayer, so we use a fast temporary file in the cache.
        """
        try:
            if not self.current_stream:
            ...
            # Use app-private cache directory for security and speed
            if ANDROID:
                try:
                    temp_dir = Context.getCacheDir().getAbsolutePath()
                except Exception as e:
                    debug_log(f"Cache dir error: {e}", "WARN")
            ...
            # Write MIDI data as quickly as possible
            debug_log(f"Generating playback file: {self.temp_file}")
            self.current_stream.write('midi', fp=self.temp_file)
            
            # Final validation before triggering Android MediaPlayer
            if os.path.exists(self.temp_file) and os.path.getsize(self.temp_file) > 10: # Min MIDI size
                if ANDROID: 
                    self._play_android()
                else: 
                    self.status_text = "Playback simulated on desktop (success)"
                    debug_log("Playback success (desktop sim)")
            else:
                self.status_text = "Error: MIDI generation failed"
                debug_log(f"MIDI file check failed: {self.temp_file}", "ERROR")
                
        except Exception as e:
            self.status_text = f"Playback Error: {str(e)}"
            debug_log(f"Play trigger error: {e}", "ERROR")

    def _play_android(self):
        try:
            debug_log("Initializing Android MediaPlayer...")
            MediaPlayer = autoclass('android.media.MediaPlayer')
            
            # Define listeners locally to avoid top-level jnius issues
            class OnPreparedListener(PythonJavaClass):
                ...
                def onPrepared(self, mp):
                    try: self.callback(mp)
                    except Exception as e: Logger.error(f"SriDAW: Prepared error: {e}")
            ...
            class OnCompletionListener(PythonJavaClass):
                ...
                def onCompletion(self, mp):
                    try: self.callback(mp)
                    except Exception as e: Logger.error(f"SriDAW: Completion error: {e}")
            ...
            class OnErrorListener(PythonJavaClass):
                ...
                def onError(self, mp, what, extra):
                    try: return self.callback(mp, what, extra)
                    except Exception as e: Logger.error(f"SriDAW: Error listener error: {e}"); return True
            ...
            def on_prepared(mp):
                try:
                    debug_log("MediaPlayer prepared, starting playback")
                    ...
                    Logger.error(f"SriDAW: Prepared callback error: {e}")

            def on_completion(mp):
                debug_log("MediaPlayer playback completed")
                self.stop_audio()

            def on_error(mp, what, extra):
                debug_log(f"MediaPlayer error: {what}, {extra}", "ERROR")
                self.stop_audio()
                return True
            ...
            debug_log(f"Android play error: {e}", "ERROR")
...
if __name__ == "__main__":
    try:
        Logger.info("SriDAW: Starting main application loop")
        app = Music21DAW()
        app.run()
    except Exception as e:
        error_msg = f"FATAL CRASH ON STARTUP: {e}\n{traceback.format_exc()}"
        Logger.error(f"SriDAW: {error_msg}")
        print(error_msg)

        # Try to show a popup if Kivy is still alive
        try:
            from kivy.uix.popup import Popup
            from kivy.uix.textinput import TextInput
            content = TextInput(
            ...
            Popup(title="Fatal Error (Copyable)", content=content, size_hint=(0.9, 0.9)).open()
        except:
            pass
